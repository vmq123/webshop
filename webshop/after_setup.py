...
def after_install():
	try:
		print("Setting up Webshop customization...")
		execute_after_install()

		click.secho("Thank you for installing Webshop customization!", fg="green")

	except Exception as e:
		BUG_REPORT_URL = "https://github.com/vmq123/utilplus/issues/new"
		click.secho(
			"Installation for Webshop customization app failed due to an error."
...
def get_custom_fields():
	"""Webshop customization specific custom fields that need to be added to the masters in ERPNext"""
	return {
		"Website Item": [
			{
                "fieldname": "custom_web_page",
				"fieldtype": "Link",
				"options":"Web Page",
				"label": _("Web Page"),
...
def update_navbar_setting():
	def update_navbar_item(parent, parenttype, parentfield, item_label, hidden):
		doc=frappe.get_doc({"doctype": "Navbar Item", "parent":parent, "parenttype":parenttype, "parentfield":parentfield, "item_label": item_label})
		doc.hidden = hidden
		doc.save(ignore_permissions=True)
	# update_navbar_item("Navbar Settings", "Navbar Settings", "help_dropdown", "Documentation",1)
	# TODO

def update_website_setting():
	settings = frappe.get_doc("Website Settings")
	settings.home_page = '/app/touropt'
	settings.app_name = 'TourOpt'
...
def update_payroll_setting():
	...
	settings.daily_wages_fraction_for_half_day = 0.5
	settings.email_salary_slip_to_employee = 0
	settings.save()

def update_system_setting():
	settings = frappe.get_doc("System Settings")
	settings.disable_system_update_notification = 1
	settings.disable_change_log_notification = 1
	settings.currency_precision = 0
